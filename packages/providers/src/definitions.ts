import type { ProviderCapability, ProviderCatalogEntry } from "@meowcode/shared";

function logo(slug: string): string {
  return `https://cdn.simpleicons.org/${slug}`;
}

export const PROVIDER_DEFINITIONS: ProviderCatalogEntry[] = [
  {
    providerId: "openai",
    displayName: "OpenAI",
...
export function getProviderDefinition(providerId: string): ProviderCatalogEntry | undefined {
  return PROVIDER_DEFINITIONS.find((p) => p.providerId === providerId);
}

export function inferCapabilitiesFromModel(
  modelId: string,
  baseCapabilities: ProviderCapability[],
  metadata?: Record<string, unknown>
): ProviderCapability[] {
  const id = modelId.toLowerCase();
  const caps = new Set<ProviderCapability>(["streaming"]);

  for (const cap of baseCapabilities) {
    if (cap !== "streaming") caps.add(cap);
  }
  ...
  if (Array.isArray(metaCaps)) {
    for (const c of metaCaps) {
      if (typeof c === "string") caps.add(c as ProviderCapability);
    }
  }
...
export function parseContextWindow(metadata?: Record<string, unknown>, modelId?: string): number | undefined {
  if (typeof metadata?.context_length === "number") return metadata.context_length;
  if (typeof metadata?.contextWindow === "number") return metadata.contextWindow;
  if (typeof metadata?.max_context === "number") return metadata.max_context;
  const id = (modelId ?? "").toLowerCase();
  if (/128k|128000/.test(id)) return 128_000;
  if (/200k|200000/.test(id)) return 200_000;
  if (/1m|1000000|1280000/.test(id)) return 1_000_000;
  if (/32k|32000/.test(id)) return 32_000;
  if (/8k|8000/.test(id)) return 8_000;
  return undefined;
}

export function parsePricing(metadata?: Record<string, unknown>): { inputPerMillion?: number; outputPerMillion?: number } {
  ...
  if (typeof metadata?.input_price_per_million === "number" || typeof metadata?.output_price_per_million === "number") {
    return {
      inputPerMillion: metadata.input_price_per_million as number | undefined,
      outputPerMillion: metadata.output_price_per_million as number | undefined
    };
  }
  return {};
}
