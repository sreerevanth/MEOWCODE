...
export class ModelRouter {
  ...
  async executeMoE<T>(
    plan: RoutePlan,
    executor: (model: ModelDescriptor) => Promise<T>,
    synthesizer: (results: T[]) => Promise<T>
  ): Promise<{ result: T; usedModels: ModelDescriptor[] }> {
    ...
    if (validResults.length === 0) throw new Error("MoE failed: all models crashed.");
    if (validResults.length === 1) return { result: validResults[0], usedModels: [chain[results.findIndex((r) => r !== null)]] };
