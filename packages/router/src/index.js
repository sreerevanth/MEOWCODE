...
export class ModelRouter {
    ...
    fromRule(rule, models, healthByProvider) {
    ...
    async executeWithFallback(plan, executor) {
    ...
    filterCandidates(request) {
        return request.models.filter((model) => {
            ...
            const capabilityMatch = (request.requiredCapabilities ?? []).every((capability) => model.capabilities.includes(capability));
            if (!healthyEnough || !capabilityMatch)
                return false;
            if (request.mode === "free_only" && !model.isFree)
                return false;
            if (request.mode === "local_only" && !model.isLocal)
                return false;
            if (request.mode === "vision" && !model.capabilities.includes("vision"))
                return false;
            if (request.mode === "reasoning" && !model.capabilities.includes("reasoning"))
                return false;
            if (request.mode === "manual_provider" && model.providerId !== request.manualProviderId)
                return false;
            if (request.mode === "manual_model" && model.id !== request.manualModelId)
                return false;
            return true;
        });
    }
