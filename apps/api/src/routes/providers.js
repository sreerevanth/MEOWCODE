...
async function assertMembership(userId, workspaceId, minRole) {
    const membership = await prisma.workspaceMember.findUnique({
        where: { userId_workspaceId: { userId, workspaceId } }
    });
    if (!membership)
        return null;
    if (minRole === "developer" && membership.role === "viewer")
        return null;
    if (minRole === "admin" && membership.role !== "owner" && membership.role !== "admin")
        return null;
    return membership;
}
...
export async function providerRoutes(app) {
    ...
    app.post("/v1/providers", async (request, reply) => {
        ...
        try {
            ...
            if (user && (user.onboardingStep === "connect_providers" || user.onboardingStep === "verify_providers")) {
                const next = conn.healthStatus === "healthy" || conn.healthStatus === "slow" || conn.healthStatus === "degraded"
                    ? "sync_models"
                    : "verify_providers";
    ...
    app.patch("/v1/providers/:id", async (request, reply) => {
        ...
        const result = await providerService.connectProvider(conn.workspaceId, {
            ...
            customHeaders: body.data.customHeaders ?? conn.customHeaders,
            timeoutMs: body.data.timeoutMs ?? conn.timeoutMs,
            retryPolicy: body.data.retryPolicy ?? conn.retryPolicy
        });
        return result.card;
    });
...
async function advanceOnboardingIfReady(userId, workspaceId) {
