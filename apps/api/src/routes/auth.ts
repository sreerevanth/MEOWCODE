...
export async function resolvePrincipal(request: FastifyRequest): Promise<SessionPrincipal | null> {
...
export async function requireAuth(request: FastifyRequest, reply: FastifyReply): Promise<SessionPrincipal | null> {
...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.get("/v1/auth/me", async (request, reply) => {
    ...
    const activeMembership =
      user.memberships.find((m) => m.workspaceId === principal.workspaceId) ?? user.memberships[0];
    ...
    const user = await prisma.user.update({
      ...
      data: {
        ...
        ...(body.data.preferences !== undefined
          ? { preferences: body.data.preferences as Prisma.InputJsonValue }
          : {}),
  ...
  app.get("/v1/auth/api-keys", async (request, reply) => {
    const principal = await requireAuth(request, reply);
    if (!principal) return;
    const workspaceId = (request.query as { workspaceId?: string }).workspaceId ?? principal.workspaceId;
  ...
  app.get("/v1/auth/oauth/:provider", async (request, reply) => {
    ...
    if (!config) return reply.status(404).send({ error: "Unknown OAuth provider" });
  ...
  app.get("/v1/auth/oauth/:provider/dev", async (request, reply) => {
    ...
    const { provider } = request.params as { provider: string };
    if (!getOAuthProvider(provider)) return reply.status(404).send({ error: "Unknown provider" });
  ...
  app.get("/v1/auth/magic-link/verify", async (request, reply) => {
    ...
    if (!email) return reply.status(401).send({ error: "Invalid or expired magic link" });

    const profile = {
      provider: "magic_link" as AuthProvider,
