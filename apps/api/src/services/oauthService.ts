...
import type { AuthProvider } from "@meowcode/auth";
...
export interface OAuthProviderConfig {
  ...
  clientId?: string;
  clientSecret?: string;
  profileMapper: (payload: Record<string, unknown>, tokens?: Record<string, unknown>) => OAuthProfile;
}
...
export function verifyOAuthState(state: string, provider: string): boolean {
  try {
    ...
    if (Number.isNaN(age) || age > 1000 * 60 * 15) return false;
...
export function getOAuthProviders(): Array<{ id: string; displayName: string; enabled: boolean }> {
...
export function buildAuthorizeUrl(config: OAuthProviderConfig, state: string): string {
  ...
  if (config.id === "google") params.set("access_type", "online");
  if (config.id === "microsoft") params.set("response_mode", "query");
...
export async function exchangeOAuthCode(
  ...
  code: string
): Promise<{ tokens: Record<string, unknown>; profile: OAuthProfile }> {
  ...
  if (!accessToken) throw new Error("OAuth token missing access_token");
  ...
  if (config.userInfoUrl) {
    ...
    if (!userRes.ok) throw new Error(`OAuth userinfo failed: ${userRes.status}`);
    userPayload = (await userRes.json()) as Record<string, unknown>;
    ...
    if (config.id === "github" && !userPayload.email) {
      ...
      if (emailsRes.ok) {
        ...
        if (primary) userPayload.email = primary.email;
      }
    }
  }
...
export async function provisionPersonalWorkspace(userId: string, name: string) {
...
export function buildWebCallbackUrl(accessToken: string, refreshToken: string): string {
...
export function buildDevOAuthProfile(provider: string): OAuthProfile {
