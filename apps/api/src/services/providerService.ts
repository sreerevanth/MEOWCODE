...
import {
  computeLatencyAggregates,
  createDefaultProviderRegistry,
  getProviderDefinition,
  resolvePlugin,
  type ProviderPlugin
} from "@meowcode/providers";
import type {
  ModelDescriptor,
  ProviderCapability,
...
function isHealthyEnough(status: ProviderHealthStatus): boolean {
  return status === "healthy" || status === "slow" || status === "degraded";
}

export class ProviderService {
  readonly defaultRegistry = createDefaultProviderRegistry();
  ...
  listCatalog(): ProviderCatalogEntry[] {
    return this.defaultRegistry.list().map((provider) => ({
      ...
      requiresApiKey: (provider as { requiresApiKey?: boolean }).requiresApiKey
    }));
  }

  private getPlugin(providerId: string, displayName: string, endpoint?: string | null): ProviderPlugin {
  ...
  private async buildConfig(connection: {
    ...
    return {
      ...
      headers: (connection.customHeaders as Record<string, string> | null) ?? undefined,
      timeoutMs: connection.timeoutMs,
      retryPolicy: (connection.retryPolicy as ProviderConnectionConfig["retryPolicy"]) ?? undefined
    };
  }
  ...
  private toCard(
    ...
    conn?: {
      ...
      availabilityPct: { toNumber(): number } | null;
      latencyConnectionMs: number | null;
  ...
  async getWorkspaceProviders(workspaceId: string): Promise<ProviderCard[]> {
  ...
  async getProviderDetail(workspaceId: string, connectionId: string): Promise<ProviderCard | null> {
  ...
  async connectProvider(
    ...
    data: {
      ...
      timeoutMs?: number;
      retryPolicy?: ProviderConnectionConfig["retryPolicy"];
    }
  ) {
    ...
    const connection = await prisma.providerConnection.upsert({
      ...
      create: {
        ...
        retryPolicy: data.retryPolicy as object | undefined,
        isConnected: false,
  ...
  private async runFullConnectionFlow(connectionId: string, plugin: ProviderPlugin): Promise<ProviderHealth> {
    const connection = await prisma.providerConnection.findUnique({ where: { id: connectionId } });
    if (!connection) throw new Error(`Provider connection ${connectionId} not found`);
  ...
  async recordHealthCheck(connectionId: string, check: () => Promise<ProviderHealth>): Promise<ProviderHealth> {
    const connection = await prisma.providerConnection.findUnique({ where: { id: connectionId } });
    if (!connection) throw new Error(`Provider connection ${connectionId} not found`);
  ...
  async verifyAndSyncProvider(connectionId: string): Promise<ProviderHealth> {
    const connection = await prisma.providerConnection.findUnique({ where: { id: connectionId } });
    if (!connection) throw new Error(`Provider connection ${connectionId} not found`);
  ...
  async runHealthCheck(connectionId: string): Promise<ProviderHealth> {
    const connection = await prisma.providerConnection.findUnique({ where: { id: connectionId } });
    if (!connection) throw new Error(`Provider connection ${connectionId} not found`);
  ...
  async runLatencyTest(connectionId: string): Promise<ProviderHealth> {
    const connection = await prisma.providerConnection.findUnique({ where: { id: connectionId } });
    if (!connection) throw new Error(`Provider connection ${connectionId} not found`);
    ...
    const aggregates = computeLatencyAggregates(
      samples.map((s) => ({ connectionMs: s.connectionMs, ttfbMs: s.ttfbMs, totalMs: s.totalMs }))
    );
  ...
  async getCapabilities(connectionId: string): Promise<{ providerId: string; capabilities: ProviderCapability[]; models: ModelDescriptor[] }> {
    const connection = await prisma.providerConnection.findUnique({ where: { id: connectionId } });
    if (!connection) throw new Error(`Provider connection ${connectionId} not found`);

    const models = await this.getAvailableModels(connection.workspaceId, connection.providerId);
    const capabilitySet = new Set<ProviderCapability>();
    for (const model of models) {
      for (const cap of model.capabilities) capabilitySet.add(cap);
    }
    const plugin = this.getPlugin(connection.providerId, connection.displayName, connection.endpoint);
    for (const cap of plugin.capabilities) capabilitySet.add(cap);
  ...
  async getStatistics(workspaceId: string, connectionId: string): Promise<ProviderStatistics> {
      ...
      for (const model of models) {
        await prisma.model.upsert({
          ...
          create: {
            ...
            metadata: model.metadata as object | undefined,
  ...
  async syncModelsOnly(connectionId: string): Promise<{ models: ModelDescriptor[]; health: ProviderHealth }> {
    const connection = await prisma.providerConnection.findUnique({ where: { id: connectionId } });
    if (!connection) throw new Error(`Provider connection ${connectionId} not found`);
  ...
  async getAvailableModels(workspaceId: string, providerId?: string): Promise<ModelDescriptor[]> {
    ...
    return dbModels.map((m) => ({
      ...
      capabilities: m.capabilities as ProviderCapability[],
      contextWindow: m.contextWindow ?? undefined,
      maxOutputTokens: m.maxOutputTokens ?? undefined,
      pricing:
        m.inputPerMillionUsd != null || m.outputPerMillionUsd != null
          ? {
  ...
  async runPeriodicHealthChecks(): Promise<void> {
    const connections = await prisma.providerConnection.findMany({
